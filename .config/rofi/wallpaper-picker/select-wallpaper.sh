...
IMAGE_PICKER_CONFIG="$HOME/.dotfiles/.config/rofi/wallpaper-picker/image-picker.razi"                                # razi config
...
if [[ -n "$SELECTED_WALLPAPER_NAME" ]]; then
  swww img "$WALLPAPER_DIR/$SELECTED_WALLPAPER_NAME" --transition-type any --transition-duration 1
fi
